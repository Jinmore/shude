 ...
 * @param $type 类型 单选 多选
**/
function sxdy($fieldid,$id_value,$type = 0){
	...
	$type = intval($type);
	if($type == 1){
		$id_value = substr(substr($id_value,1),0,-1);
		$id_arr = explode(",",$id_value);
		$id_num = count($id_arr)-1;
	}
	...
	foreach($shuxing_arr as $_k) {
		...
		$option[$k] = $v[0];
		if($type == 1){
			foreach($id_arr as $m => $c) {
				if($k==$c){$fh_value .= $option[$k];$fh_value .= "、";}
			}
		}else{
			if($k==$id_value){$fh_value = $option[$k];}
		}
	}
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			...
			$multipage .= ' <a href="'.pageurl($urlrule, $pre_page, $array).'" class="a1">'.L('previous').'</a>';
			if($curr_page==1) {
...
function catpos($catid, $symbol=' > '){
	...
	foreach($arrparentid as $catid) {
		...
		if($y_catid == $catid){
			$pos .= $symbol.'<a href="'.$url.'" class="on">'.$category_arr[$catid]['catname'].'</a>';
		}else{
