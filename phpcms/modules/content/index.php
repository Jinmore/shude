...
class index {
	...
	public function show() {
		...
		if($CATEGORYS[$CATEGORYS[$catid][parentid]][parentid]==6){
	...
	public function lists() {
		...
		$arrparentid=$CATEGORYS[$catid][arrparentid];
		$check_arr=explode(",", $arrparentid);

		if(in_array(6,$check_arr) && $CATEGORYS[$catid][child]==0){
			$_pri=$this->_course_priv($catid);
			switch ($_pri) {
					case '1':
					//  echo $_pri;
	...
	public function public_checkAu(){

		$catid = intval($_GET['cid']);
		$jieduan=intval($_GET['chapter']);	
		$CATEGORYS = getcache('category_content_1','commons');
		$cid=$CATEGORYS[$catid]['parentid'];

		if(!$catid||!$jieduan) exit('-2');
		$_userid = $this->_userid;
		if(!$_userid) exit('-1');//未登录
		...
		if($result){//若课程没有购买
			...
			$chapter_infd=$exam_chapter_db->select(array('uid'=>$_userid,'courseid'=>$cid,"chapterid"=>$catid,"status"=>0));		
			if($chapter_infd){//若通过,且已开通
				exit('1');
			}else{
				exit('-3'); 
			}
