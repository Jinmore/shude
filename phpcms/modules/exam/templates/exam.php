...
class exam extends admin
{
	...
	function edit(){
		...
		if($_POST['dosubmit']){
			...
			if($info){
				...
				$data['question']=safe_replace($info['question']);
				$data['selecte']=array2string($info['selecte']);
			...
			$list_select=string2array($selecte);
	...
	function importQ(){
		...
		{
			//提交后处理
			$classId=$_POST['classid']?intval($_POST['classid']):showmessage(L('choose').L('class'));
			//获取课程分类
			$courseid=$_POST['courseid']?intval($_POST['courseid']):showmessage(L('choose').L('course'));
			//获取章节分类
			$knowsid=$_POST['knowsid']?intval($_POST['knowsid']):showmessage(L('choose').L('chapter'));
			...
			if($result){
					...
					if($res&&is_array($res)){
						...
						foreach ($res as $k => $v) {
							...
							$datas['selecte']=array2string(explode("###", $v[1]));
							$datas['correct']=$v[2];
							...
							$datas['class']=$v[3]?intval($v[3]):$classId;//
							$datas['courseid']=$v[3]?intval($v[3]):$courseid;//导入表格中是否需要填写章节？？
							$datas['knowsid']=$v[4]?intval($v[4]):$knowsid;							
