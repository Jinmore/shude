...
<div class="pad_10">
<form action="?m=exam&c=exam&a=edit" method="post" name="myform" id="myform">
<table cellpadding="2" cellspacing="1" class="table_form" width="100%">
	...
	<tr>
		...
		<select  id="classtype" name="exam[class]">

		<option value="0">请选1111择系列</option>
		<?php
		  $i=0;
		  foreach($classinfo as $catid=>$va){
		  $i++;
		?>
		<option value="<?php echo $catid;?>" <?php if($class==$catid) echo 'selected';?>><?php echo $va['catname'];?></option>
		<?php }?>
		</select>	


		<select  id="coursetype" name="exam[courseid]">
		...
		<?php
		  ...
		  $i++;
		?>
		<option value="<?php echo $catid;?>" <?php if($courseid==$catid) echo 'selected';?>><?php echo $va['catname'];?></option>
		<?php }?>
		</select>	

		<select  id="coursechapter" name="exam[knowsid]">
...
<textarea name="exam[question]" cols="50" rows="8" id="content"><?php echo $question;?></textarea>
<?php echo form::editor('content','basic','','','',1,1)?>

		<!-- <input type="text" name="exam[question]" id="link_name"
			size="50" class="input-text" value="<?php echo $question;?>"> --></td>
	</tr>
	...
	<tr>
		...
		<?php 
		$i=1;
			if($list_select&&is_array($list_select)){
				foreach ($list_select as $key => $value) {
		?>		
			<div class="option" id="<?php echo $key;?>"><span>
			<input type="text" name="exam[selecte][]" value="<?php echo $value;?>" id="selecte" size="50" class="input-text"><input type="button" value="<?php echo "&nbsp;&nbsp;".L('del')."&nbsp;&nbsp;"?>"  onclick="del('<?php echo $key;?>')" class="button"/></span></div>			
		<?php
		$i++;
				}
			}
	...
	<tr>
		...
		<td>
			...
			<input type="radio" value="A" <?php if(strtoupper($correct)=='A') echo 'checked';?> name="exam[correct]"> A 
			<input type="radio" value="B" <?php if(strtoupper($correct)=='B') echo 'checked';?> name="exam[correct]"> B
			<input type="radio" value="C" <?php if(strtoupper($correct)=='C') echo 'checked';?> name="exam[correct]"> C
			<input type="radio" value="D" <?php if(strtoupper($correct)=='D') echo 'checked';?> name="exam[correct]"> D 
			<input type="radio" value="E" <?php if(strtoupper($correct)=='E') echo 'checked';?> name="exam[correct]"> E 
...
<tr>
		<th></th>
		<td><input type="hidden" name="forward" value="?m=exam&c=exam&a=edit"> <input
		type="submit" name="dosubmit" id="dosubmit" class="dialog"	value=" <?php echo L('submit')?> "></td>
	</tr>
