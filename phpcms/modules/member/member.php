...
class member extends admin {
	...
	function search() {
		...
		$db2=pc_base::load_model('member_model');
		foreach($memberlist as $k=>$v){
			$db2->set_model($v['modelid']);
			$userinfo = $db2->get_one(array('userid'=>$v[userid]));		

			$memberlist[$k][sale]=$userinfo[sale];
			$memberlist[$k][rent]=$userinfo[rent];
			$memberlist[$k][lx]=$userinfo[classtype];
			$memberlist[$k][title]=$userinfo[title];
		}
		...
		//	$basicinfo['groupid'] = $_POST['info']['groupid'];
			...
			$status = $this->client->ps_member_edit($info['username'], $info['email'], '', $info['password'], $userinfo['phpssouid'], $userinfo['encrypt']);
  			if($status >= 0) { 
				...
				if($userinfo) {
					if($basicinfo['modelid']==34){//若类型更改
						if($_POST[info][classtype]!=$userinfo[classtype]){
							$modelinfo[sale]='';
							$modelinfo[rent]='';
						}
					}
	...
	public function public_relationlist() {
		...
		if(!isset($_GET['userid'])){
			...
			$membermodelinfo = $this->db->get_one(array('userid'=>$userid));
			if($type=='rent'){
				if($membermodelinfo['classtype']==3){
					$catid=3;
				}elseif ($membermodelinfo['classtype']==2) {
					$catid=14;
				}elseif($membermodelinfo['classtype']==1){
					$catid=13;
				}	
			...
			if(isset($_GET['keywords'])) {
				...
				$where = $where ?  ' status=99'.$where : ' status=99';
				...
				$tablename = $content_db->table_name;	
				
				$infos = $content_db->listinfo($where,'',$page,12);
	...
	public function public_getjson_ids() {
		...
		if($type=='rent'){
				...
				$relations=$membermodelinfo[rent];	
			}else{
			  ...
			  if($membermodelinfo['classtype']==3){
				...
				$relations=$membermodelinfo[sale];	
			}
		$siteids = getcache('category_content','commons');
		$siteid = $siteids[$catid]?$siteids[$catid]:1;
		$CATEGORYS = getcache('category_content_'.$siteid,'commons');	
		...
		$content_db->set_model($modelid);
		$tablename = $content_db->table_name;
		$content_db->table_name = $tablename.'_data';

		if($relations) {
			$relation = str_replace('|', ',', $relations);
			$relation = trim($relation,',');
			$where = "id IN($relation)";
			$infos = array();
			$content_db->table_name = $tablename;
			$datas = $content_db->select($where,'id,title');
			foreach($datas as $_v) {
				$_v['sid'] = 'v'.$_v['id'];
	...
	function sendMsg() {
		if(isset($_POST['dosubmit'])) {
			...
			include $this->admin_tpl('member_send');
		}
	}
