...
class index extends foreground {
	...
	private $times_db,$style,$clistinfo=array();
	...
	public function init() {
		...
		$listsinfo=array();
		foreach($CATS as $catid => $va) {
			if($va['parentid']==6){
				$listsinfo[$catid]=$va;
			}
		}
		$where=array("uid"=>$memberinfo[userid]);
		$page = isset($_GET['page']) ? $_GET['page'] : 1;
		$clistinfo = $this->exam_course_db->listinfo($where, 'id DESC', $page,10);		
		
		$pages = $this->exam_course_db->pages;	
		
		include template($this->style, 'index');
	}
	
public function wapinit() {
		$memberinfo = $this->memberinfo;		
		//初始化phpsso
		$phpsso_api_url =  $r = $this->db->get_one(array('userid'=>intval($_userid))); if($r[username]!=$_username){ return '2'; } $this->_init_phpsso(); 
		//获取头像数组
		...
		$where=array("uid"=>$memberinfo[userid]);
		$page = isset($_GET['page']) ? $_GET['page'] : 1;
		$clistinfo = $this->exam_course_db->listinfo($where, 'id ASC', $page,10);
		
		$pages = $this->exam_course_db->pages;	
		
		include template($this->style, 'coursemsg');
	}
	...
	public function account_manage() {
		...
		include template($this->style, 'account_manage');
	}

	public function public_edit_av(){
		$dataimg=$_POST['img'];
		$memberinfo = $this->memberinfo;
		$uid=param::get_cookie('_userid');
		
		// if(empty(intval($uid))){
		// 	echo json_encode(array('msgcode'=>1101,'msg'=>'未知用户'));
		// 	die;
		// } 
		$dir1 = ceil($uid / 10000);
		$dir2 = ceil($uid % 10000 / 1000);
			...
			//创建图片存储文件夹
		//$avatarfile = APP_PATH.'phpsso_server/uploadfile/ '.'avatar/';
		$avatarfile = pc_base::load_config('system', 'upload_path').'avatar/';
		//
		$base_filepath=substr($avatarfile, 0,strpos($avatarfile , uploadfile));

		$dir = $base_filepath.'phpsso_server/uploadfile/avatar/'.$dir1.'/'.$dir2.'/'.$uid.'/';
		if(!file_exists($dir)) {
				mkdir($dir, 0777, true);
		}
			//存储flashpost图片
		if (preg_match('/^(data:\s*image\/(\w+);base64,)/',$dataimg, $result)){
			  $type = $result[2];
			  $filename = $dir."180x180.jpg";			  
			 if (file_put_contents($filename, base64_decode(str_replace($result[1], '', $dataimg)))){
			 	pc_base::load_app_class('image','','0');
				$image = new image(1,0);
				$image->thumb($filename, $dir.'30x30.jpg', 30, 30);
				$image->thumb($filename, $dir.'45x45.jpg', 45, 45);
				$image->thumb($filename, $dir.'90x90.jpg', 90, 90);
				 $mdb = pc_base::load_model('sso_members_model');
				$mdb->update(array('avatar'=>1), array('uid'=>$uid));
			     echo json_encode(array('msgcode'=>1100,'msg'=>$filename.$base_filepath));
			}else{
				 echo json_encode(array('msgcode'=>1102,'msg'=>$filename));
			}

		}else{
			  echo json_encode(array('msgcode'=>1101,'msg'=>$filename));
		}
	}

	public function account_manage_avatar() {
		$memberinfo = $this->memberinfo;
		$random=sys_auth('wz010_'.random(6));
		//初始化phpsso
		$phpsso_api_url =  $r = $this->db->get_one(array('userid'=>intval($_userid))); if($r[username]!=$_username){ return '2'; } $this->_init_phpsso(); 
		...
		include template($this->style, 'account_manage_avatar');
	}
	...
	public function account_manage_password() {
		if(isset($_POST['dosubmit'])) {
			...
			include template($this->style, 'account_manage_password');
		}
	}
	//更换手机号码
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			include template($this->style, 'login');
		}
	}
	...
	*/
	...
	public function public_forget_password_mobile () {
		...
		//处理提交申请，以手机号为准
			...
			$siteid = isset($_REQUEST['siteid']) && trim($_REQUEST['siteid']) ? intval($_REQUEST['siteid']) : 1;
			$siteinfo = siteinfo($siteid);
 			include template($this->style, 'forget_password_mobile');
		}
	}
	//通过用户名找回密码
