...
?>
    <tr>
		...
		<td align="center">
			<a href="javascript:edit(<?php echo $v['userid']?>, '<?php echo $v['username']?>')">[<?php echo L('edit')."基本信息"?>]</a>

			<a href="javascript:allocated(<?php echo $v['userid']?>, '<?php echo $v['username']?>')">[<?php echo L('分配课程')?>]</a>

			<a href="?m=member&c=course&a=courselist&userid=<?php echo $v['userid']?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>&uname=<?php echo $v['username']?>">[<?php echo L('查看课程')?>]</a>
...
function allocated(id,name){
	window.top.art.dialog({id:'edit'}).close();
	window.top.art.dialog({title:'<?php echo "分配课程"?>《'+name+'》',id:'edit',iframe:'?m=member&c=course&a=addCourse&userid='+id,width:'700',height:'500'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
