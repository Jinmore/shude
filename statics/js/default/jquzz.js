(function($) {
    $.fn.jquizzy = function(settings) {
        var defaults = {
            questions: null,
            startImg: 'statics/images/default/start.gif',
            endText: '已结束!',
            shortURL: null,
            sendResultsURL: null,
            resultComments: {
        ...
        for (questionsIteratorIndex = 0; questionsIteratorIndex < config.questions.length; questionsIteratorIndex++) {
        	/**题目*/
            contentFob += '<div class="slide-container"><div class="question-number">' + (questionsIteratorIndex + 1) + '/' + config.questions.length + '</div><div class="question">' + config.questions[questionsIteratorIndex].question + 'id:'+config.questions[questionsIteratorIndex].id+ '</div><ul class="answers"><div class="time-item"><strong class="hour_show">0时</strong><strong class="minute_show">0分</strong><strong class="second_show">0秒</strong></div>';
            for (answersIteratorIndex = 0; answersIteratorIndex < config.questions[questionsIteratorIndex].answers.length; answersIteratorIndex++) {
                contentFob += '<li data-id='+config.questions[questionsIteratorIndex].id+'##'+(answersIteratorIndex+1)+'>' + config.questions[questionsIteratorIndex].answers[answersIteratorIndex] + '</li>';
            }
            //倒计时
        ...
        superContainer.find('.nav-start').click(function() {
			...
			$('.slide-container').each(function() {
				$(this).find('ul').append('<li style="display:none"></li>');
				$(this).find('ul').find('li:last').addClass('selected');
				$(this).find('ul').find('li:last').attr('data-id',$(this).find('ul').find('li:first').attr('data-id')+'n')
            });
            return false;
        });
        superContainer.find('.next').click(function() {
            if ($(this).parents('.slide-container').find('li.selected').length === 0) {
				$(this).parents('.slide-container').find('ul').find('li:last').addClass('selected');
                /*notice.fadeIn(300);
                return false;*/
            }
            notice.hide();
        ...
        superContainer.find('.final').one('click',function() {
            /*if ($(this).parents('.slide-container').find('li.selected').length === 0) {
                notice.fadeIn(300);
                return false;
            }*/
            superContainer.find('li.selected').each(function(index) {
                ...
                userAnswers.push($(this).parents('.answers').find('li.selected').attr('data-id'));
            });
            progressKeeper.hide();
            var resultSet = '';
            if (config.sendResultsURL !== null) {
                ...
                $.getJSON(config.sendResultsURL, {an: myanswers}, function(json) {
                    if (json == null) {
                        ...
                        resultSet = '<h2 class="qTitle">' + judgeSkills(json.score) + '<br/> 您答对了: ' + json.score + '</h2><div class="jquizzy-clear"></div>' + resultSet + '<div class="jquizzy-clear"></div>';
